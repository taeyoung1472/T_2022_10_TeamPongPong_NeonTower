...
public class SwordMove<T> : BossState<T> where T : Sword
{
    ...
    private GameObject dangerZoneObj = null;
    ...
    public IEnumerator SwordDash()
    {
        ...
        {
            case 1:
                dangerZoneObj = DangerZone.DrawCircle(dangerzonePos, stateMachineOwnerClass.radius[a - 1] * 2, 1.5f); // 외곡 찍기
                break;
            case 4:
                dangerZoneObj = DangerZone.DrawCircle(dangerzonePos, stateMachineOwnerClass.radius[a - 1] *2, 1.5f);  //연속베기 (콤보1)
                break;
            case 5:
                dangerZoneObj = DangerZone.DrawCircle(dangerzonePos, stateMachineOwnerClass.radius[a - 1] * 2, 1.5f);  //연속베기 (콤보2)
                break;
            case 6:
                dangerZoneObj = DangerZone.DrawCircle(dangerzonePos, stateMachineOwnerClass.radius[a - 1] * 2, 1.5f); //원 범위 공격 ( 텔, 대쉬해서 공격)
                break;
        ...
        {
            case 2:
                dangerZoneObj = DangerZone.DrawArc(dangerzonePos, d, 5, new Vector3(2, 0.1f, 8), 1.25f); // 내려찍기
                stateMachineOwnerClass.arcangle = 15f;
                break;
            case 3:
                dangerZoneObj = DangerZone.DrawArc(dangerzonePos, d, 4, new Vector3(3.5f, 0.1f, 14), 1.75f); //회오리 참격베기
                stateMachineOwnerClass.arcangle = 18f;
                break;
            case 7:
                dangerZoneObj = DangerZone.DrawArc(dangerzonePos, d, 2, new Vector3(7, 0.1f, 10), 1.75f); //발도 기술
                stateMachineOwnerClass.arcangle = 40f;
                break;
        }
        
        stateMachineOwnerClass.dangerZone = dangerZoneObj;
